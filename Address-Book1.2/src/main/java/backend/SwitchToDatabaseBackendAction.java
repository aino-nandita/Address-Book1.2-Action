...
public class SwitchToDatabaseBackendAction implements Action {
	...
	public SwitchToDatabaseBackendAction(Menu menu, BackendManagerMenuAction bmanager) {
		this.bmm=bmanager;
	...
	public void performAction() {
		checkInstance();
		setCurrentBackend();
		bmm.setBackendhelper(new DatabaseAddressBookHelper(bmm));
		System.out.println("\nBackend Helper: " + bmm.getBackendhelper()+ "\n");
	}
	
	public void checkInstance() {
		for(Backend b : bmm.list) {
			if(b instanceof DatabaseBackend) {
				System.out.println("\n"+ bmm.list.indexOf(b) + ". "+ b);
			} 
		}
	}
